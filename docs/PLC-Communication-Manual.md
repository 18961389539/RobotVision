...
> **协议类型**：TCP/IP ASCII 纯文本逗号分隔行协议（应答以 `\n` 结尾；请求有无换行均可）  
> **最新修订**：2026-08  
> **破坏性变更**：v3 **不再使用 `TRIGGER,` 前缀**。触发行直接发配方名或序列号。旧报文 `TRIGGER,A01` 返回 `ERR,1013`。 
...
| **字符编码** | **ASCII（7-bit）** | 禁止在通讯报文中夹带非 ASCII 字符（含中文、全角标点）；错误消息中的非 ASCII 会被剥离 |
| **帧结束符** | **应答 `\n` (LF, 0x0A)** | 视觉→PLC 的每条应答以 `\n` 结尾。PLC→视觉的请求**推荐**同样加 `\n`（兼容 `\r\n`）；**不带换行也可以**，服务端在约 100ms 无后续字节后按完整命令处理 |
| **数值格式** | **ASCII 十进制浮点数** | 小数点必须为英文点号 `.`，负号为 `-`。**严禁包含逗号**（逗号作为字段分隔符） |
| **并发与流控** | 单连接串行半双工交互 | 每个客户端连接上遵循“**一发一收**”原则；收到当前应答后方可发送下一条请求 |
| **超时建议** | 视觉处理超时 **90000ms**（启动下限；json 更小会被抬到 90s） | PLC 端 Socket 接收超时建议 **≥ 180000ms**（≥ 2 × 视觉超时）。**连接默认不因空闲断开**（`IdleTimeoutMs=0`） |
...
| :--- | :--- | :--- |
| `PING\n` | `PONG\n` | 链路心跳检测（可选；默认不断开空闲连接） |
| `STATUS\n` | `OK,ready\|busy,队列深度,队列上限,上次耗时ms[,连续失败,联锁0\|1]\n` | 触发前管线状态查询，避免任务堆叠 |
| `配方名\n` 或 `序列号\n` | `OK,X1,Y1,RZ1,...,配方名,目标数,耗时ms\n` | **基础触发**：固定相机 / 免位姿校验工位。成功行**不含**置信度 |
| `配方名,X,Y,RZ\n` | 同上 | **位姿触发**：末端相机（Eye-in-Hand）防呆引导 |

> **注**：除 `PING` / `STATUS` / `CLEARINHIBIT` 外，任意一行都当触发行。拼写错误会按配方名解析，通常 `ERR,1001`（不是 1000）。旧格式 `TRIGGER,A01` → `ERR,1013`。
...
```
                    ┌── 1. 固定相机 (Eye-to-Hand) ──► 基础触发: A01
                    │
工业相机安装工况 ────┤
                    │
                    └── 2. 末端相机 (Eye-in-Hand) ──► 位姿触发: A01,X,Y,RZ
                                                      (带 1012 位姿防呆拦截)
```
...
### 3.2 选型建议
* **固定在机架上的相机**：使用 `A01\n` 即可；
* **安装在机械臂末端的相机**：**必须**使用 `A01,X,Y,RZ\n`；
* **产线程序统一化**：PLC 可以统一封装「配方名,X,Y,RZ」功能块。固定相机工位即使上报了位姿，视觉服务端也会识别并自动忽略，无任何负面影响。
* **不要**再发 `TRIGGER,` 前缀。
...
配方名,X,Y,RZ\n
```
* 示例：`A01,152.340,-210.500,45.000\n`

旧格式 `TRIGGER,A01` / `TRIGGER,A01,X,Y,RZ` 一律 `ERR,1013`。
...
```text
OK,X1,Y1,RZ1,X2,Y2,RZ2,...,配方名,目标数量,总耗时ms\n
```

#### 字段拆解结构：
$$\underbrace{\text{OK}}_{1},\underbrace{X_1,Y_1,RZ_1}_{\text{第1个目标}},\dots,\underbrace{X_N,Y_N,RZ_N}_{\text{第N个目标}},\underbrace{\text{A01}}_{\text{配方名}},\underbrace{N}_{\text{目标数}},\underbrace{\text{ElapsedMs}}_{\text{耗时}}\text{\textbackslash n}$$

| 序号 | 字段名 | 类型 | 单位 | 示例值 | 说明 |
| :---: | :--- | :---: | :---: | :---: | :--- |
| **1** | 成功响应头 | STRING | - | `OK` | 识别为成功报文 |
| **2~1+3N** | 目标 X/Y/RZ | REAL | mm / deg | `102.356` | **紧跟 OK 顺序读取** $3 \times N$ 个坐标字段 |
| **倒数第 3** | 配方名 | STRING | - | `A01` | 回显实际执行的配方 ID（审计/对账） |
| **倒数第 2** | **目标数量 $N$** | INT | - | `2` | 校验坐标段个数；$N=0$ 时为 `OK,配方名,0,耗时ms` |
| **末尾** | 处理耗时 | INT | ms | `342` | 从收到触发到计算完成输出的总耗时 |

> 📐 **目标坐标系随工位标定方式而异**：
> - 外参/多项式（Robot 空间）工位 → 机器人基坐标系 mm（可直接引导机器人）；
> - **比例标定工位（无标定板回退）** → **图像平面毫米**（原点=图像左上角，X 向右 Y 向下，与视觉软件预览画面同向），
>   需上位机自行换算，不可直接作为机器人坐标使用。
> 详见《README》标定章节的管线分发优先级说明（多项式 > 外参 > 比例）。

#### 报文示例：
* **单目标检出 ($N=1$)**：
  ```text
  OK,250.410,-120.330,90.150,A01,1,215\n
  ```
* **双目标检出 ($N=2$)**：
  ```text
  OK,100.120,50.200,-15.300,180.400,60.100,164.700,A01,2,380\n
  ```
...
| :---: | :--- | :--- | :---: | :--- |
| **1000** | `UnknownCommand` | 极少见（如 CLEARINHIBIT 未就绪） | 否 | 拼写错误通常是 **1001**，不是 1000。 |
| **1001** | `UnknownRecipe` | 配方不存在或配方名包含非法字符 | 否 | **停机报警**。核对触摸屏配方号与视觉内命名。 |
...
| **1004** | `NotCalibrated` | 工位未做外参/多项式/比例标定，或分辨率不匹配 | 否 | **停机报警**。提示视觉工程师完成标定。 |
...
| **1008** | `Timeout` | 视觉处理总耗时超过配置上限（默认 90000ms）| **先查后试** | **不可立即重发**。发 `STATUS` 确认 `ready` 后再重试。PLC 收包超时须大于视觉超时。 |
...
| **1015** | `RecipeDisabled` | 配方存在但被停用（Enabled=false） | 否 | **报警通知操作员**。在视觉界面启用该配方后重试。 |
| **1016** | `InvalidRecipeConfig` | 配方参数/引用校验失败（相机/模型/阈值等） | 否 | **报警通知视觉工程师**。打开配方页按提示修正配置。 |
| **1017** | `AssetMismatch` | 模型或标定档案与配方钉扎的 SHA-256 不一致 | 否 | **报警**。核对文件后在配方页重新钉扎哈希。 |
| **1018** | `ProcessUnhealthy` | 同配方连续过程失败达到联锁阈值 | 否（排除后） | **停机检查**光源/来料/相机。发 `CLEARINHIBIT`（可带配方名或序列号）或通信页「解除联锁」后再触发。联锁拒绝不占队列、不计入失败次数。 |
| **1019** | `RefineFailed` | 分割检出目标，但精修未过质量门（模板匹配/卡尺/孔槽/头尾） | **可重试 1 次** | 与 1007 不同：画面里有件。查照明与配方精修方法。连续失败通知视觉工程师，不要当漏检抛料。 |
| **1099** | `InternalError` | 视觉内部未捕获异常（含配方文件 IO 故障） | 否 | **报警并记录时间点**，供视觉工程师排查日志。 |
...
CASE nState OF
    ...
    STATE_SEND_TRIGGER:
        ...
        // 格式: A01,100.230,200.500,45.000\n  （不要 TRIGGER, 前缀）
        sSendBuffer := sRecipeName;
    ...
    STATE_PARSE_DATA:
        ...
        IF FIND(sRecvBuffer, 'OK,') = 1 THEN
            // 拆分字段: OK,102.356,-88.412,45.120,A01,1,342
...
1. **网络连通性测试**：
   ...
   * 发送 `PING\n`，应立即收到 `PONG\n`。
2. **报文结束符检查**：
   * 视觉应答以 `0x0A`（`\n`）结尾，PLC 按行解析即可。请求**建议**同样加 `\n`（立即处理）；不加换行也可以，服务端约 100ms 静默后同样执行（如只发 `#1` 或 `PING`）。
3. **小数与符号格式**：
...
5. **OnArm 末端工位防呆测试（必测项）**：
   * 将机器人移动到拍照点后，手动在报文中将 X 坐标篡改偏移 $+2.0\text{mm}$ 发送 `配方名,X+2,Y,RZ\n`；
