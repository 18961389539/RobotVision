...
- 模板匹配**失败归因诊断**:不过门时输出可读原因(最佳 NCC vs 阈值差多少/无候选/第二峰歧义),试触发与留档可见,不再裸报 1019
- 模板匹配**第二峰歧义门**(`Template.MaxSecondPeakRatio`):周期性纹理/近对称特征出现次峰与主峰几乎同分时判歧义拒绝,防相似误匹配
- 模板匹配 **XY 亚像素中心**:匹配峰抛物线插值 ±0.5px,高分辨率相机下定位精度从 1px 提升到亚像素级(强纹理方向实测 ~0.01px)
- 标定向导支持直接新建比例标定档案
