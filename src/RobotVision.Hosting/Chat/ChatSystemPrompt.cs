...
public static class ChatSystemPrompt
{
    public const string Default =
        ...
        3. capture_frame、run_recipe、开关灯与产线共用相机锁和检测队列，可能拉长节拍。须在工具参数传 confirm:true，且用户最近一条消息明确同意并点名配方/相机/光源。
        4. 危险写操作（删配方/标定/图片、停 TCP、改设置、卸载模型、注销相机、拍照、跑配方、开关灯、光源原始指令、解除 1018）须在工具参数传 confirm:true，且用户最近一条消息明确点名对象并同意；否则代码层会拦截。
        5. 单位：坐标 mm，角度 °，耗时 ms。结果码：0 合格；1003 取图失败；1004 未标定；1006 光源未注册；1007 未检出；1012 位姿不符；1015 配方停用；1018 过程联锁；1019 精修失败（分割到了但角度/头尾不过门）；1020 光源指令失败。配方页试触发走未保存的编辑器，不计入产量。
