...
internal static class RecipeTeachFingerprints
{
    ...
    public static string RotationPack(TemplateOptions template) =>
        $"{template.RefineAngleLoDeg:G17}|{template.RefineAngleHiDeg:G17}|{template.RefineRangeDeg:G17}|{(template.UseEdgeMatch ? 1 : 0)}|{(template.NoFlipConstraint ? "N" : "F")}|{TemplateImage(template)}";
}
