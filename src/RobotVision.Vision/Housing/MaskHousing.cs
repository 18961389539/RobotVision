...
public static class MaskHousing
{
    ...
    public static HousingFrame Fit(IReadOnlyList<Point2f> contour)
    {
        ...
        var obb = new HousingFrame(rect.Center, warp, longLen, shortLen);

        var robust = RotatedRectFitter.Fit(pts, warp, RectFitOptions.Default with { ClipEndPoints = 2 });
        if (!robust.Ok)
            return obb;

        // 角口径固定 MinAreaRect（与转正/卡尺一致）；仅当鲁棒框显著更贴轮廓时才换中心/尺寸。
        var obbRms = ResidualRms(pts, obb);
        if (robust.RmsPx + 0.2 < obbRms)
        {
            return new HousingFrame(
                new Point2f((float)robust.Center.X, (float)robust.Center.Y),
                warp,
                robust.LongLen,
                robust.ShortLen);
        }
    ...
    private static double ResidualRms(Point2f[] pts, HousingFrame frame)
    {
        var halfLong = frame.LongLen * 0.5;
        var halfShort = frame.ShortLen * 0.5;
        var rad = frame.WarpAngleDeg * Math.PI / 180.0;
        ...
        return pts.Length == 0 ? double.MaxValue : Math.Sqrt(sum / pts.Length);
    }
    ...
    /// <summary>MinAreaRect 长边方向 + Canon，与 <see cref="MaskTemplateMatcher.UprightCrop"/> 同口径。</summary>
