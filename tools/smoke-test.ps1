...
# 用法: .\tools\smoke-test.ps1 [-Server 127.0.0.1] [-Port 9999] [-Commands @("PING","A03")]
param(
...
foreach ($cmd in $Commands) {
    ...
    try {
        ...
        elseif ($cmd -eq "STATUS") {
            if ($reply -notmatch "^OK,(ready|busy),") { Write-Host "[失败] STATUS 应返回 OK,ready|busy,..."; $failed++ }
        }
        else {
            if ($reply -notmatch "^(OK|ERR),") { Write-Host "[失败] 触发行应答应以 OK, 或 ERR, 开头"; $failed++ }
            elseif ($reply -match "^ERR,") { Write-Host "[警告] 触发行返回错误（配方/相机/标定相关，按产线预期判断）" }
        }
    } catch {
