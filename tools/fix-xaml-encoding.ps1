$root = Join-Path $PSScriptRoot '..\src\RobotVision.Wpf'
$fixes = @(
    @('结果?/', '结果图 /'),
    @('结果图?ROI', '结果图 / ROI'),
    @('首次取?测试', '首次取图 / 测试'),
    ...
    @('<Run Text=" · ?" /><Run Text="{Binding BytesIn', '<Run Text=" · 入" /><Run Text="{Binding BytesIn'),
    @('<Run Text=" · ?" /><Run Text="{Binding BytesOut', '<Run Text=" · 出" /><Run Text="{Binding BytesOut'),
    @('<TextBlock Text="" Margin="0,0,0,4" FontSize="11" ToolTip="宽度，0~1 相对比例" />', '<TextBlock Text="W" Margin="0,0,0,4" FontSize="11" ToolTip="宽度，0~1 相对比例" />'),
    @('<TextBlock Text="" Margin="0,0,0,4" FontSize="11" ToolTip="高度，0~1 相对比例" />', '<TextBlock Text="H" Margin="0,0,0,4" FontSize="11" ToolTip="高度，0~1 相对比例" />')
)

function Fix-WhPx([string]$content) {
    if ($content -notmatch '\?\(px\)') { return $content }
    $labels = @('W','H')
    $i = 0
    return [regex]::Replace($content, '\?\(px\)', { param($m) $l = $labels[$i % 2]; $script:i++; return "$l(px)" })
}

$count = 0
Get-ChildItem -Path $root -Filter *.xaml -Recurse | ForEach-Object {
    $text = [System.IO.File]::ReadAllText($_.FullName, [System.Text.UTF8Encoding]::new($false))
    $orig = $text
    foreach ($pair in $fixes) { $text = $text.Replace($pair[0], $pair[1]) }
    $script:i = 0
    $text = Fix-WhPx $text
    if ($text -ne $orig) {
